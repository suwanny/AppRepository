...
def main(argv): 
  if len(argv) < 6 : 
    print "usage: AppPlotMulti.py root stat_name dir_count file_name file_count"
    print "example: ./AppPlotMulti.py ~/data hbase 3 data_appscale-image 5"
    sys.exit(0)

  root_dir = argv[1]
  statistics_name = argv[2]
  ...
  for index in range(1, dir_count+1):
    dirnames.append(root_dir + "/" + str(index))
  ...
  for index in range(file_count):
    ...
    result.print_responseTime_persecond( root_dir )
  ...
  file_response = open(root_dir + "/" + "response.dat", "w")
  file_transactions = open(root_dir + "/" + "transactions.dat", "w")
